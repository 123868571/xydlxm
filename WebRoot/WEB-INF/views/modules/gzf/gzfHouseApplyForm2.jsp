...
<script type="text/javascript">
	$(document).ready(function() {
		...
		if (formContent != "") {
			...
			$('input:radio[name=education][value=' + obj.education + ']').attr('checked', true);
		}
		//==初始化操作end==
		...
		$(".ok").click(function() {
			...
			formContent.companyAddr = $('#companyAddr').val();
			formContent.companyName = $('#companyName').val();
			formContent.organizationCode = $('#organizationCode').val();
			formContent.laborContractStart = $('#laborContractStart').val();
			formContent.socialInsuranceStart = $('#socialInsuranceStart').val();
			formContent.education = $('input:radio[name=formContent\\.education]:checked').val();
			formContent.collegeFinishTime = $('#collegeFinishTime').val();
			formContent.college = $('#college').val();

			if (isUndefined(formContent.education)) {
				formContent.education = -1;
			}
			...
			$('.membertr').each(function() {
				...
				member.memberMaritalStatus = -1;
				member.memberCompany = _this.find('.memberCompany').val();
				member.memberIncome = '';
				member.memberRelation = -1;
									...
									<div class="portlet box blue">
										<div style="border-bottom: 1px solid rgb(245, 245, 245);"
											...
											<div style="color: rgb(77, 144, 254);" class="caption">
												<i class="icon-edit1"></i><span class="whiteFont">新就业大学毕业生申请使用</span>
											</div>
										</div>
												...
												<div class="span12">
													<div class="control-group">
														<table border="1" style="width: 100%">
															<thead>
																<th colspan="7"><h3>公 共 租 赁 住 房 申 请 表Ⅱ</h3> <br />编号:
																	${serial} <br /> <br />（新就业大学毕业生申请使用）</th>
															</thead>
															<tbody>
																<tr>
																...
																<tr>
																	...
																	<td>单位注册地址</td>
																	<td><input type="text" id="companyAddr"
																		value="${dto.formContent.companyAddr }"></td>
																</tr>
																<tr>
																...
																<tr>
																	<td colspan="2">组织机构代码证</td>
																	<td colspan="3"><input type="text"
																		style="width: 356px" id="organizationCode"
																		value="${dto.formContent.organizationCode }"></td>
																	<td></td>
																	<td></td>
																</tr>
																<tr>
																...
																<tr>
																	<td colspan="2">学历</td>
																	<td colspan="3"><form:radiobuttons
																			path="dto.formContent.education"
																			items="${fns:getDictList('apply_form_education')}"
																			itemLabel="label" itemValue="value"
																			htmlEscape="false" class="required" /></td>
																	<td>本科毕业时间</td>
																	<td><input type="text" readonly="readonly"
																		id="collegeFinishTime" maxlength="20"
																		class="input-large Wdate required"
																		value="${dto.formContent.collegeFinishTime}"
																		onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});" /></td>
																</tr>
																<tr>
																	<td colspan="2">毕业学校</td>
																	<td colspan="3"><input type="text" id="college"
																		style="width: 356px"
																		value="${dto.formContent.college}"></td>
																	<td></td>
																	<td></td>
																</tr>
																<tr>
																...
																<tr>
																	<td colspan="7">
																		<table width="100%" height="100%">
																			<thead>
																				<th class="bi-right bi-bottom" width="25%">配偶或父母
																				</th>
																				<th class="bi-right bi-bottom" width="25%">联系电话</th>
																				<th class="bi-right bi-bottom" width="25%">身份证号码</th>
																				<th class=" bi-bottom" width="25%">工作单位</th>
																			</thead>
																			<tbody>
																				...
																				<c:forEach items="${dto.familyMembers}" var="member">
																					<tr id="otherMember_${member.id}" class="membertr"
																						...
																						<td class="bi-right bi-bottom"><input
																							type="text" class="memberPhone" maxlength="11"
																				...
																				<c:forEach begin="1" end="${emptyMembersSize}"
																					...
																					<tr class="membertr" data-id="">
																						...
																						<td class="bi-right bi-bottom"><input
																							type="text" class="memberPhone" maxlength="11"></td>
																						<td class="bi-right bi-bottom"><input
																							type="text" class="w80 memberIdcard"
																							maxlength="18"></td>
																						<td class="bi-right bi-bottom"><input
