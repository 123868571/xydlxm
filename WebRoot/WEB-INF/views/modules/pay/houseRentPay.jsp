	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#search").click(function(){
				...
				var spePaymentId = "<%=PaymentConst.SPECIAL_PAYMENT.HOUSE_RENT%>";
				...
				postJson(url, datas,function(state, json){
					if (state == "success") {
						...
						//console.log("json"+json.pageHtml);
						//console.log("rs_html"+rs_html);
						$("#resText").html(rs_html);
						$("#pagination").html(json.pageHtml);
						$("#pageSize").val(json.payList.pageSize);
						$("#pageNo").val(json.payList.pageNo);
					}else{
						//alert("出错了！！:"+data.msg);
					}
				}); 
		...
		function query(){
			$("#searchForm").submit();
			if("${gzfAccount.id}" != ""){
				$('#payBtn').removeAttr("disabled"); //移除disabled属性 
			}else{
				$("#payBtn").attr("disabled",false); 
			}
		}*/

		function goToPay(url, datas){
			var accountId = $("#accountId").val();
			if(accountId ==null || accountId==""){
				alert("请先查询住户信息!");
				return false;
			}
			//$("#houseForm").attr("action","${ctx}/pay/gzfAccount/houseRentPayForm");
			//$("#houseForm").attr("action","${ctx}/pay/gzfAccount/houseRentPayForm?accountId=${gzfAccount.id}&amount=${gzfAccount.fee}");
			$("#houseForm").submit();
		}
	</script>
	</head>
		...
		<div id="payInfo" class="portlet-body">
			<div class="row-fluid">
				<div class="span12">
					<form:form id="searchForm" modelAttribute="gzfAccount" action="${ctx}/pay/gzfAccount/getHouseRentHomePageInfo" method="post" class="form-search">
						...
						<div class="span12">
							<div class="portlet box blue">
								<div class="portlet-title">
									<div class="caption"><i class="icon-edit"></i><span class="whiteFont">房租缴费</span></div>
										<div class="tools">
									...
									<div class="portlet-body form">
										<form:form id="houseForm" style="margin:0" modelAttribute="gzfPayment" action="${ctx}/pay/gzfAccount/houseRentPayForm" method="post" class="form-horizontal">
										<form:hidden path="accountId" id="accountId"/>
										<form:hidden path="custId"/>
										<form:hidden path="spePaymentId"/>
										<form:hidden path="effectDateStr"/>
										<form:hidden path="amount"/>
										<form:hidden path="amountYuan"/>
											...
											<div class="tab-pane profile-classic row-fluid">
												<ul class="unstyled span6 ">
													...
													<li class="no-padding no-margin">
														...
														${householdInfo.gzfHouseInfo.gzfPalaces.name}${householdInfo.gzfHouseInfo.buildNum}楼${householdInfo.gzfHouseInfo.unit}单元${householdInfo.gzfHouseInfo.room}号
														</c:when>
													...
													<li class="no-padding no-margin">
														<span class="pad padWidth">房租单价:</span>
														<div class="pad">${householdInfo.paymentStandard.rentUnitPrice}</div>
														<span class="pad">元/m<sup>2</sup></span>
													</li>
													<li class="no-padding no-margin">
												...
												<ul class="unstyled span6 ">
													<li class="no-padding no-margin">
														<span class="pad padWidth">房费到期日期:</span>
														<div class="pad padFont">
														<c:choose>
															<c:when test="${lastBalance.expireDate != null}">
																<fmt:formatDate value="${lastBalance.expireDate}" pattern="yyyy-MM-dd"/>
															</c:when>
															<c:otherwise></c:otherwise>
														</c:choose>
														</div>
													...
													<li>
														...
														<shiro:hasPermission name="pay:gzfPayment:pay">
														<%--<button id="rentPayment" class="btn red"><i class="icon-yen"></i>--%>
															<%--<a href="${ctx}/pay/gzfPayment/houseRentPayForm?id=${gzfPayment.id}">房租缴费</a>--%>
														<%--</button>--%>
														<button id="payBtn" type="button" class="btn red" onclick="goToPay()"><i class="icon-yen"></i>房租缴费</button>
														</shiro:hasPermission>
													</li>
												</ul>
											</div>
					...
					<div class="portlet">
						<div class="portlet-title bottom-line">
							<div class="caption" style="color: rgb(77, 144, 254);"><i class="icon-reorder"></i>房租交费历史信息</div>
