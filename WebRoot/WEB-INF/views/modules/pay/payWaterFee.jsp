	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#search").click(function(){
				...
				var spePaymentId = "<%=PaymentConst.SPECIAL_PAYMENT.WATER%>";
		...
		<div id="payInfo" class="portlet-body">
			<div class="row-fluid">
				<div class="span12">
					<form:form id="searchForm" modelAttribute="gzfAccount" action="${ctx}/pay/gzfAccount/getWaterFeeHomePageInfo" method="post" class="form-search">
						...
						<div class="span12">
							<div class="portlet box blue">
								<div class="portlet-title">
									<div class="caption"><i class="icon-edit"></i><span class="whiteFont">水缴费</span></div>
										<div class="tools">
									...
									<div class="portlet-body form">
										<form:form id="houseForm" style="margin:0" modelAttribute="gzfPayment" action="${ctx}/pay/gzfPayment/houseRentPayForm?accountId=${gzfAccount.id}&amount=${gzfAccount.fee}" method="post" class="form-horizontal">
										<form:hidden path="id"/>
										...
										<div class="tabbable tabbable-custom tabbable-full-width">
											<div class="tab-pane profile-classic row-fluid">
												<ul class="unstyled span6 ">
													...
													<li class="no-padding no-margin">
														<span class="pad padWidth">建筑面积:</span>
													...
													<li class="no-padding no-margin">
														<span class="pad padWidth">水单价:</span>
														<div class="pad">${householdInfo.paymentStandard.waterFee}</div>
														<span class="pad">元/吨</span>
													</li>
													<li class="no-padding no-margin">
														<span class="pad padWidth">水表数(吨):</span>
														<div class="pad">${gzfAccount.remarks}</div>
														<span class="pad">(未缴始-末)</span>
													</li>
													<li class="no-padding no-margin">
														<span class="pad padWidth">水费金额:</span>
														<div class="pad">
														<c:choose>
														<c:when test="${gzfAccount.fee != null}">${gzfAccount.fee}
														</c:when>
														<c:otherwise></c:otherwise>
														</c:choose>
														</div>
														...
														<c:when test="${gzfAccount.fee != null}">
															<a href="${ctx}/pay/gzfAccount/listAcctItem?accountId=${gzfAccount.id}&acctItemTypeId=<%=PaymentConst.ACCT_ITEM_TYPE.WATER%>">查看帐单</a>
														</c:when>
														<c:otherwise></c:otherwise>
														</c:choose>
													</li>
					...
					<div class="portlet">
						<div class="portlet-title bottom-line">
							<div class="caption" style="color: rgb(77, 144, 254);"><i class="icon-reorder"></i>水交费历史信息</div>
