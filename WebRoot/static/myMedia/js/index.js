var Index = function () {
    ...
    return {
        ...
        initCharts: function (activities) {
            if (!jQuery.plot) {
                return;
            }
            ...
            function showTooltip2(title, x, y,  contents, xValue, yValue) {
                $('<div id="tooltip" class="chart-tooltip"><div class="date">' + title + '<\/div><div class="label label-important">' + yValue+ '%<\/div><\/div>').css({
                    position: 'absolute',
                    display: 'none',
                    top: y - 100,
                    width: 75,
...
//                    $("#y").text(pos.y.toFixed(2));
                    if (item) {
                        if (previousPoint2 != item.dataIndex) {
                            ...
                            var x = item.datapoint[0].toFixed(2),
                                y = item.datapoint[1].toFixed(2);
                            showTooltip2('学历', x, y, "", item.datapoint[0], item.datapoint[1]);
                        }
                    }
                });
